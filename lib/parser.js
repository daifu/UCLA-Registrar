   ...
   courses.findAllTerms(function(error, allTerms){
      if (allTerms.length === 0) {
         ...
         request({url: 'http://www.registrar.ucla.edu/schedule/schedulehome.aspx'}, function(err, response, body){
            ...
            jsdom.env({
               html: body,
               scripts: ['//code.jquery.com/jquery-1.6.min.js']
            }, function(err, window) {
      ...
      if (subjects === null || subjects === undefined) {
         request({url: subject_url}, function(err, response, body){
            ...
            function(err, window) {
               //Use jQuery just as in a regular HTML Page
               var $ = window.jQuery,
               title = $('#ctl00_BodyContentPlaceHolder_crsredir1_lblSAHeaderNormal').text().trim(),
               $class_opts = $('#ctl00_BodyContentPlaceHolder_crsredir1_lstCourseNormal').children(),
               cls = [];

               if (title.length === 0) {
                  title = $('#ctl00_BodyContentPlaceHolder_crsredir1_lblSAHeaderTentative').text().trim();
               }
      ...
      if (course === null || course === undefined) {
         request({url: class_url}, function(err, response, body){
            ...
            jsdom.env({
               ...
               //Use jQuery just as in a regular HTML Page
               var $ = window.jQuery,
                   title = $('.SAHeaderDarkGreenBar').text().trim(),
               ...
               $course_head.each(function(i, v) {
                  ...
                  if ($tmp.parents('.SAHeaderDarkGreenBar').length > 0) {
                     course_title = $(v).text().trim();
                  } else if ($tmp.next('.fachead').length > 0) {
                     var instuctor = $tmp.next('.fachead').text().trim().replace(/\s+/g, ""),
                         instuctor_link = instuctor.split(',')[0];
                     course_sec[sec_counter] = {
                        'section': $tmp.text().trim(),
                        'instuctor': instuctor,
                        'section_link': $tmp.text().trim().replace(/\s+/g, "")
                     };
                     ...
                     if ($tr.hasClass('.dgdClassDataHeader')) {
                        ...
                        table_data.each(function(di, dv){
                           ...
                           switch(td_data.attr('class')) {
                              case IDClassName:
                                 var id_num = td_data.find('a').text().trim();
...
Parser.prototype.courseDescription = function(term, sub, classid, class_url, idNum, subjectName, callback) {
   request({url: class_url}, function(err, response, body){
      ...
      jsdom.env({
         ...
         //Use jQuery just as in a regular HTML Page
         var $ = window.jQuery,
             desp = $("#ctl00_BodyContentPlaceHolder_subdet_lblCourseDescription").text().trim().strip_tag(),
   ...
   request({url: prof_url}, function(err, response, body){
      ...
      jsdom.env({
         ...
         var $ = window.jQuery,
             ...
             depart = $($('.breadcrumbs li')[2]).text().trim().replace(/\//, ''),
             avgs_key = [
                 'Effective',
                 'Easy',
                 'Concerned',
         ...
         if (avg_reviews.length === 0) {
            $results.each(function(i, v){
               ...
               $li.each(function(li, lv){
                  ...
                  avg_reviews[li] = {
                     'key': avgs_key[li],
                     'value': $(lv).text().trim().replace(/[a-z]+/i, '').trim()
                  };
               });
               avg_reviews[avgs_key_len] = {
                  'key': avgs_key[avgs_key_len],
                  'value': $(v).find('.overall').text().trim().replace(/[a-z]+/i, '').trim()
               };
