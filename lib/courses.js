...
//===========STARRED=============//
Courses.prototype.findAllStarredByEmail = function(email, callback) {
   this.getCollection('users', function(error, users_collection) {
      if (!error && users_collection) {
         users_collection.findOne({email: email}, function(error, course){
            if (!error && course) {
               callback(null, course);
            } else {
               callback(error);
...
Courses.prototype.saveStarredByEmail = function(isAdd, email, term, sub, classid, callback) {
   var self;
   if (this === undefined) {
      courses = new Courses();
      self = courses;
   ...
   self.getCollection('users', function(error, users_collection){
      if (!error && users_collection) {
         users_collection.find({email:email}).toArray(function(error, user){
            user = user.pop();
            if (user === undefined || user === null) {
               user = {};
               user.email = email;
               self.findAllTerms(function(error, terms){
                  if (!error && terms) {
                     user.courses = terms;
                     user.courses.map(function(term){
                        term.subjects = [];
                        return term;
                     });
                     users_collection.insert(user);
                     user.updated_at = new Date();
                     users_collection.update({email: email}, user, true, false);
                     callback(null, user);
                  }
               });
            } else {
               user.updated_at = new Date();
               if (term && sub && classid) {
                  self.findCourse(term, sub, classid, function(error, course) {
                     if (!error && course) {
                        user.courses.forEach(function(ele) {
                           if (ele.key === course.term) {
                              var isUnique = true,
                                  course_title = course.title;
                              for (var i = ele.subjects.length - 1; i >= 0; i--) {
                                 if(ele.subjects[i].title === course_title) {
                                    isUnique = false;
                                    i = -1;
                                    if (!isAdd) {
                                       ele.subjects.splice(i, 1);
                                    }
                                 }
                              }
                              if (isUnique && isAdd) {
                                 ele.subjects.push(course);
                              }
                           }
                        });
                        users_collection.update({email: email}, user, true, false);
                        console.log("saveStarredByEmail updated");
                        callback(null, user);
                     }
                  });
               } else {
